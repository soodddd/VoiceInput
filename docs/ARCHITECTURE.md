...
> **0.1.4 当前实现补充（2026-09-05）**：录音会话由专用线程拥有 WASAPI `cpal::Stream`，实时回调只写入受限缓冲区和原子音量快照，`audio-level` 由 50 ms 采样线程发送。当前已验证的分发方式是 onedir ZIP；本文中 NSIS、安装器和早期事件名属于设计规划，当前状态以源码和 `VALIDATION_v0.1.4-preview.*.md` 为准。
